...
You will need [bar-aint-recursive](https://github.com/LemonBoy/bar) (for the panel) and [dzen](https://github.com/robm/dzen) (for the popups). bspwm is *not* required, you will only have to rewrite chunks of the `panel` and `panel-monitor` scripts if you don't use it.

To have the same panel as me, put all the scripts in your $PATH. Sometimes, it might need another script I have put in my `~/.bin`, so look for it in the appropriate [place](https://github.com/tatou-tatou/dotfiles/tree/master/.bin).
...
*Note:* The BAR developper was lazy and haven't implemented a way to increase the maximum number of clickable areas dynamically. In the source code, there is a `#define N 20` somewhere, **increase** that value if you need more.
...
The startmenu is made by mygtkmenu. It appears when clicking on the blank space on the left of the workspace list.

You can easily add an icon or more space by editing the `start_icon` variable in `panel-settings`.

*Note:* If you use bspwm and want to use a right click menu, you may want to take a look [there](https://github.com/tatou-tatou/Themes/blob/master/Mousse/MouseLaunch).
...
It will display a notification in the panel. If during three seconds there are no new notifications, the music will be displayed.
...
With those lines in your sxhkdrc, pressing the *XF86AudioRaiseVolume* will raise the PCM channel volume by 1 decibel, display the new volume value and -after three seconds of inactivity- show the mpd status again.
...
####With buttons
`not-stat` is also called by the scripts in the `Monitor` subdir when clicking on various buttons or when using the different sliders. Read the next section.
...
All of this is generated by `panel-monitor` and the two dzen scripts. It appears when you do a **left** click on the `not-stat` blue icon.
...
- The variables `Fr="AZERTY"` and `Us="QWERTY"` are just to have a message for not-stat (see the `layoutCmd` variable), they can be removed safely if you don't use that.
...
The same for [the script I use](https://github.com/tatou-tatou/dotfiles/blob/master/.bin/mbp_backlight) to change the backlight. If you want to use it, you will need to adapt it to your system.

###How to change buttons?
####Remove a button
If you think a button is useless (let's say, the keyboard layout changer or the PCM channel volume controls), just remove it from the `for Button in Vp Vm Kb Sc` line and it won't appear anymore. You can then remove the associated code from the `case` statement.

You will also want to remove the unused lines from `dslider` and `dslider-content`.

If you removed a button that has a slider, you may want to edit `not-stat` as well.

####Change a button name
Edit the items in the `for Button in Vp Vm Kb Sc` and in the `case` statement.

You **MUST** make the same edits in `dslider` and `dslider-content`! If you use `not-stat` and edited a button that spawn a slider, you have to edit that script too, else you won't be able to move the slider indicator position through keyboard alone.

####Add a button
Just add an item to `for Button in Vp Vm Kb Sc` and you will have a new button. It **must** be two characters long!

To configure what appears when you focus it, add a condition to the `case`. That condition must create content for the `message` variable.
...
The most important part is to understand how the popup calculates where it should appear. Try to understand what's inside the $Opened and $Current variables and you will get it.

In practice, you won't have to do anything else than add those two lines in `dslider`:
- one to adjust the coordinates if the pink button was clicked while *another* pink button was already focused. It should be something like `<button title>) xpos=$(($xpos - <width of the button label>)) ;;`
- one to adjust the coordinates when it's the *arrow* that was clicked.

The code is heavily commented, so the place where to insert those lines will be obvious.

Edit `dslider-content` to make your actual slider. You just have to provide a way to get the current value, a way to get the max value and a way to set the current value.
...
*Note:* the new gif API broke `meh`, making the coverart script unusable right now. It is not that good anyway.

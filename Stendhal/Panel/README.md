...
The same for [the script I use](https://github.com/tatou-tatou/dotfiles/blob/master/.bin/mbp_backlight) to change the screen brightness. If you want to use it, you will need to adapt it to your system.
...
Then edit the name in the `case` statement.

**IMPORTANT: You *must* make the same edits to `dslider` and `dslider-content`!**

Additionally, if you use `not-stat` and edited a button that spawn a slider, you have to edit that script too, else you won't be able to move the slider indicator position through keyboard alone anymore.
...
To configure what appears when you click on it, add a condition to the `case`. That condition must create content for the `message` variable.
...
The most important part is to understand how the popup calculates where it should appear. Try to understand what are the `$Opened` and the `$Current` variables and you will get it.

In practice, you only have to add two lines in `dslider`:
...
Edit `dslider-content` to actually create your own slider. You have to provide a way to get the *current* value, a way to get the *max* value and a way to *set* the current value.
